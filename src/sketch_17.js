// 距離によって色が変わるやつをやってみたい
// 三角形の各頂点から対辺への距離を計算して(h_a, h_b, h_c)、
// 計算結果に対してもそれを計算して(d_a, d_b, d_c)、
// 1-(h/d)の各値でR,G,Bを決めて表現したら面白そう。

// rを固定して回転だけ
// step0: 外側にも内側にも距離関数を実装する
// step1: パターン決定時に各頂点の座標を求める
// step2: 各頂点と対辺との距離を求める(rは0.5で固定)(h_a, h_b, h_c)
// step3: 距離の集合をvec3形式で送る
// step4: イテレーション終了時におけるベクトルについて対辺との距離を求める(d_a, d_b, d_c)
// step5: reflectionの戻り値をvec3にして(1-d/h)の配列を返すようにする。終わり。
...
"    p = p + vec2(0.0, 1.0);" +
"  }else{" +
"    if(norm > 0.999){ return vec3(0.7, 0.7, 0.7); }" +
